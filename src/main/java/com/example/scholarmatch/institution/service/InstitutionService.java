...
public class InstitutionService {
    ...
    public Institution register(InstitutionRequest request) {
        ...
        if (request.getRegistrationNumber() != null
            ...
            throw new IllegalArgumentException("An institution with this registration number already exists");
        }
        ...
        institution.setPasswordHash(hashPassword(request.getPassword()));
        ...
        institution.setPanNumber(request.getPanNumber());
        institution.setRegistrationNumber(request.getRegistrationNumber());
        institution.setVerificationStatus("PENDING");

        Institution saved = institutionRepository.save(institution);
        verificationLogService.recordSubmission(saved.getInstitutionId());
        return saved;
    }
    ...
    public List<Institution> getByVerificationStatus(String status) {
        return institutionRepository.findByVerificationStatus(status);
    }

    public Institution update(Long institutionId, InstitutionRequest request) {
        ...
        institutionRepository.update(institutionId, existing);
