...
public class NotificationRepository {
    ...
    public Notification save(Notification n) {
        String sql = "INSERT INTO notification " +
                "(recipient_type, student_id, institution_id, notification_type, related_scholarship_id, " +
                "title, message, is_read, is_email_sent) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)";
    ...
    public Optional<Notification> findById(Long notificationId) {
        ...
        try {
            return Optional.ofNullable(jdbcTemplate.queryForObject(sql, rowMapper, notificationId));
    ...
    public List<Notification> findByStudentId(Long studentId) {
        String sql = "SELECT * FROM notification WHERE student_id = ? ORDER BY created_at DESC";
        return jdbcTemplate.query(sql, rowMapper, studentId);
    }

    public List<Notification> findByInstitutionId(Long institutionId) {
        String sql = "SELECT * FROM notification WHERE institution_id = ? ORDER BY created_at DESC";
        return jdbcTemplate.query(sql, rowMapper, institutionId);
    }

    public List<Notification> findUnreadByStudentId(Long studentId) {
        String sql = "SELECT * FROM notification WHERE student_id = ? AND is_read = FALSE ORDER BY created_at DESC";
        return jdbcTemplate.query(sql, rowMapper, studentId);
    }

    public List<Notification> findUnreadByInstitutionId(Long institutionId) {
        String sql = "SELECT * FROM notification WHERE institution_id = ? AND is_read = FALSE ORDER BY created_at DESC";
        return jdbcTemplate.query(sql, rowMapper, institutionId);
    }
    ...
    /** Marks every unread notification for one student as read. Returns rows affected. */
    public int markAllAsReadByStudentId(Long studentId) {
        String sql = "UPDATE notification SET is_read = TRUE WHERE student_id = ? AND is_read = FALSE";
        return jdbcTemplate.update(sql, studentId);
    }

    public int countUnreadByStudentId(Long studentId) {
        String sql = "SELECT COUNT(*) FROM notification WHERE student_id = ? AND is_read = FALSE";
        Integer count = jdbcTemplate.queryForObject(sql, Integer.class, studentId);
        return count == null ? 0 : count;
    }

    public int markEmailSent(Long notificationId) {
        String sql = "UPDATE notification SET is_email_sent = TRUE WHERE notification_id = ?";
        return jdbcTemplate.update(sql, notificationId);
    }
    ...
    public boolean existsByStudentScholarshipAndType(Long studentId, Long scholarshipId, String notificationType) {
        String sql = "SELECT COUNT(*) FROM notification WHERE student_id = ? AND related_scholarship_id = ? AND notification_type = ?";
        Integer count = jdbcTemplate.queryForObject(sql, Integer.class, studentId, scholarshipId, notificationType);
